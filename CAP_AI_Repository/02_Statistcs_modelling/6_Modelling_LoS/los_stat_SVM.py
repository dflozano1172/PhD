...
y_data_bal = [ews for idx, ews in enumerate(list(list(zip(*X_data_16_18[1]))[1])) if idx in idx_final]
...
y_train      = np.array([los_codes(x) for x in y_data_bal])
...
params = {'kernel' :(0,3), 'degree':(1, 3), 'gamma': (0,1), 'c_p': (1,10)}
