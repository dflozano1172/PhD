...
adms = X_data_16_18[0]['admission_id'].unique().tolist()
...
adms_val = X_data_19_20[0]['admission_id'].unique().tolist()
...
# Sampling Train Set
# =========================================
no_samples = 950 #Number of samples per class
...
y_train = np.array(ews_Y_train)
y_valid = np.array(ews_Y_valid)
y_test = np.array(ews_Y_test)
...
params = {'max_depth': (5,15), 'booster': (0,2), 'learning_rate':(0.1, 0.5), 'gamma':(0,5), 
          'tree_method': (0,1), 'num_parallel_tree':(1,100), 'n_estimators':(50,90)}
...
nn_bo.maximize(init_points=3, n_iter=6)
