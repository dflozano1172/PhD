...
train_set = X_data_16_18[0][X_data_16_18[0]['age_at_admin'] <= 65].copy()
adm_ts = train_set['admission_id'].unique().tolist()
idx_final = [idx for idx, adm in enumerate(adm_list) if (adm in adm_ts)]
...
y_train    = np.array([ews for idx, ews in enumerate(X_data_16_18[1]) if idx in idx_final])
EWS_bands  = lambda x: 0 if x == 0 else 1 if x == 1 else 2 if x == 2 else 3 if x == 3 else 4
y_train = np.array([EWS_bands(x) for x in np.array(y_train)])
...
params = {'kernel' :(0,3), 'degree':(1, 4), 'gamma': (0,1), 'c_p': (0,40)}
